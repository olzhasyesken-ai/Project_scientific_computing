...
%% Testing
fprintf('\n=== Test 1: Checking monotonicity of analytic solutions ===\n');

for k = 1:length(shape_hs)
    fprintf('--- Shape parameter h = %d ---\n', shape_hs(k));

    test_monotonicity(analytic_outage(k,:), ...
                      sprintf('analytic outage (k=%d)', k));

    test_monotonicity(numeric_analytic_outage(k,:), ...
                      sprintf('numerical analytic outage (k=%d)', k));

    test_monotonicity(analytic_capacity(k,:), ...
                      sprintf('Upper bound capacity (k=%d)', k));
end


fprintf('\n=== Test 2: Accuracy (MSE < 1e-4) ===\n');

threshold_mse = 1e-4;

for i = 1:length(shape_hs)
    if mse(i) < threshold_mse
        fprintf('Accuracy test for numerical solution, shape parameter = %d: PASSED ✓  (MSE = %.3e)\n', ...
                 shape_hs(i), mse(i));
    else
        fprintf('Accuracy test for numerical solution, shape parameter = %d: FAILED ✗  (MSE = %.3e)\n', ...
                 shape_hs(i), mse(i));
    end
end
...
%             sum_{n=0}^∞ [(-1)^n * (z*theta)^(n/2) * gamma((alpha+n)/2 + r) / 
%             (n! * (alpha+n) * beta^n)]
%
% Inputs:
...
function test_monotonicity(vec, name)
    d = diff(vec);

    if all(d >= 0)
        fprintf('Test for %s: monotonically increasing ✓\n', name);
    elseif all(d <= 0)
        fprintf('Test for %s: monotonically decreasing ✓\n', name);
    else
        fprintf('Test for %s: NOT monotonic ✗\n', name);
    end
end
